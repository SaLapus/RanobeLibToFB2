...
import { Data as Chapter } from "../../types/api/Chapter";
...
describe("Chapter Download Integration", () => {
  ...
  it("downloads selected chapters when clicking download button", async () => {
    // Setup mocks
    const mockChapterContent: Partial<Chapter> = {
    ...
    vi.mocked(api.fetchChapter).mockResolvedValue(
      mockChapterContent as Chapter
    );
    vi.mocked(parseChapters.default).mockResolvedValue(mockParsedContent);
  ...
  it("handles errors in the download process", async () => {
    ...
    vi.mocked(api.fetchChapter).mockRejectedValue(new Error("Network error"));
    ...
    await waitFor(() => {
      expect(consoleError).toHaveBeenCalledWith(new Error("Network error"));
  ...
  it("skips unchecked chapters during download", () => {
    const { getByText } = renderWithProviders(<Title />, {
      initialState: {
        slug: "test-slug",
        titleInfo: mockTitleInfo,
